...
1. **M1 — Skeleton & voice proof:** scaffold, `start.bat`, server, settings screen (API key), and a bare live-voice loop — talk to the model, hear it back, see the transcript. *De-risks everything.*
2. **M2 — PRD Review session:** setup screen, persona definitions, system-instruction assembly, live session screen with timer/transcript/end flow, audio recording + usage/cost capture, session saved to `data/sessions/`.
3. **M3 — Evaluation:** rubric prompt, eval call, report screen with cost summary + recording player, persistence.
4. **M4 — Playbook:** notes library CRUD, per-session toggles, playbook-adherence section in the eval.
5. **M5 — History & next challenges:** history screen, score trends, one-click follow-up challenges.
6. **M6 — Polish & resilience:** hardened resumption/GoAway handling, push-to-talk, intensity tuning, CSV export, coming-soon cards.
